...
FILE* log_file = NULL;
const char* log_file_name = "../Common/Tree/logs/lang_log_file.html";
...
void PrintGraphizNode(FILE* graphiz_file,  Node* node)
{
    ...
    {
        char* rus_name = ConvertEncoding(node->lexeme.str.name);
        fprintf(graphiz_file, "| VALUE: %s", rus_name);
        free(rus_name);
    }
...
void FillLogFile(char* image_file_name,  Tree* tree, int file_counter)
{
    ...
    fprintf(log_file, "\n\n<img src=imgs/image%d.svg width=%dpx>\n\n",
...
const char* GetNodeTypeName(Node* node)
{
    ...
    switch(node->type)
    {
        ...
        case KEY_PUTCHAR:           return "draw";
        case SPEC_SYM_SPACE:        return "space";
        case SPEC_SYM_ENTER:        return "enter";
...
const char* GetNodeColor(Node* node)
{
    ...
    switch(node->type)
    {
        ...
        case KEY_PUTCHAR:           return "#a7f020ff";
        case SPEC_SYM_SPACE:        return "#a7f020f1";
        case SPEC_SYM_ENTER:        return "#a7f020f1";
...
static char* GetNewDotCmd(int file_counter)
{
    ...
    snprintf(str_command, sizeof(str_command),
            "dot -Tsvg ../Common/Tree/logs/imgs/image%d.txt "
            "-o ../Common/Tree/logs/imgs/image%d.svg",
...
static char* GetNewImageFileName(int file_counter)
{
    ...
    snprintf(str_file_counter, sizeof(str_file_counter),
             "../Common/Tree/logs/imgs/image%d.txt", file_counter);
...
char* ConvertEncoding(char* win1251)
{
    ...
    {
        unsigned char sym = (unsigned char)win1251[i];
    ...
    {
        unsigned char sym = (unsigned char)win1251[win1251_i];

        if (sym < 128)
            utf8[utf8_i++] = (char)sym;
